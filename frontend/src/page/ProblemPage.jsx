      ...
      case "description":
        return (
          <div className="prose max-w-none">
            <p className="text-lg mb-6">{problem.data.description}</p>

            {problem.data.examples && (
              <>
                <h3 className="text-xl font-bold mb-4">Examples:</h3>
                {Object.entries(problem.data.examples).map(
                  ([lang, example], idx) => (
                    <div
                      key={lang}
    ...
    <div className="min-h-screen bg-linear-to-br from-base-300 to-base-200 max-w-7xl w-full">
      <nav className="navbar bg-base-100 shadow-lg px-4">
        <div className="flex-1 gap-2">
          ...
          <div className="mt-2">
            <h1 className="text-xl font-bold">{problem.data.title}</h1>
            <div className="flex items-center gap-2 text-sm text-base-content/70 mt-5">
              <Clock className="w-4 h-4" />
              <span>
                Updated{" "}
                {new Date(problem.data.createdAt).toLocaleString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })}
        ...
        <div className="flex-none gap-4">
          ...
          >
            {Object.keys(problem.data.codeSnippets || {}).map((lang) => (
